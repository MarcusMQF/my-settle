    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                ...
                setReports([...mappedReports, ...initialReports]);
            } catch (error) {
                console.error("Failed to fetch dashboard", error);
                // Ensure we at least show mock data if API fails
                setReports((prev) => {
                    // If prev has reports (e.g. from init), keep them, ensuring start with init if empty
                    if (prev.length === 0) return initialReports;
                    return prev;
                });
            }
        };
